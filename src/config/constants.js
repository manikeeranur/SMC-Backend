...
// IST. Once ACTIVE, a trade exits on SL/Target, the 3:20pm square-off, or
// the stagnant timeout below — there is no VWAP-close exit.
...
const VWAP930_SL_PCT      = 6;   // stop loss  −8%
const VWAP930_TARGET_PCT  = 30;  // target     +24%
...
const VWAP930_ENTRY_START_MINUTE = 36;
// At most 2 entries per day: the 1st at whichever checkpoint first qualifies,
// and — only if that one exits with a status in VWAP930_REENTRY_STATUSES
// (i.e. SL) — one re-entry at the next qualifying checkpoint. A TARGET hit
// on the 1st trade ends the day right there (no 2nd entry); whatever the 2nd
// entry does (SL or TARGET), the day ends there too — there is never a 3rd.
...
const VWAP930_STAGNANT_HOURS      = 3;
const VWAP930_STAGNANT_MAX_POINTS = 30;
// Exit statuses that are allowed to trigger a re-entry: SL or a stagnant
// timeout — both are "this setup didn't work, try again" outcomes. A TARGET
// hit ends the day immediately, never allowing another entry.
